...
void initTheWholeWorld (World &w, Ball &ba) ///Initialisation de la balle dans le monde et de tous les murs du jeu
{
    ...
    { ///Initialisation des murs du jeux (environement)
        AddWall(w, 50, 450, 150, 350);
        AddWall(w, 250, 300, 200, 150);
        AddWall(w, 310, 150, 410, 150);
        AddWall(w, 360, 400, 360, 250);
        AddWall(w, 680, 450, 580, 350);
        AddWall(w, 580, 200, 480, 300);
    }
...
void colisionWindow(Ball &b)
{
  {///Colision sur fenêtre
    ...
    if (b.p.y < 0)  ///colision fenêtre gauche
    {
        b.p.y = b.p.y;
        b.v.y = -b.v.y;
        b.v = FRICTION * b.v;
